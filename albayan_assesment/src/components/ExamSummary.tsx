  ...
  export const ExamSummary: React.FC<ExamSummaryProps> = ({
    ...
    const getStatusCounts = () => {
      ...
      if (allTestResponses && allTestResponses.length > 0) {
        console.log('All test responses for counting:', allTestResponses);
        
        // Count from all accumulated responses using both Selopt and questionStatus
        allTestResponses.forEach((response, index) => {
          console.log(`Response ${index}:`, response);
          
          // Check if response has questionStatus information (new format)
          if (response.questionStatus) {
            const status = response.questionStatus;
            if (!status.visited) {
              counts.notVisited++;
            ...
            // Fallback to old logic for responses without questionStatus
            const isAnswered = isResponseAnswered(response);
            console.log(`Response ${index} is answered:`, isAnswered);
    ...
    const isResponseAnswered = (response: any): boolean => {
      if (!response || (!response.Selopt && response.Selopt !== 0)) {
        return false;
      }
      ...
      // For all question types, any non-empty content is considered answered
      // This is more inclusive and should catch more answered questions
      return true;
    };
    ...
    console.log('- QuestionStatus availability:', allTestResponses?.filter(r => r.questionStatus).length || 0, 'out of', allTestResponses?.length || 0);
